 ...
 *
 * This is an example script for DSDV manet routing protocol.
 *
 * Authors: Pavel Boyko <boyko@iitp.ru>
 */

#include "ns3/dsdv-module.h" // Change to DSDV module
...
/**
 * \ingroup dsdv-examples // Update to DSDV
 * \ingroup examples
 * \brief Test script.
 *
 ...
 */
class DsdvExample // Update class name
{
  public:
    DsdvExample();
    /**
...
{
    DsdvExample test; // Update to DSDV
...
//-----------------------------------------------------------------------------
DsdvExample::DsdvExample() // Update class name
    : size(10),
      step(50),
      totalTime(100),
      pcap(true),
...
bool
DsdvExample::Configure(int argc, char** argv) // Update class name
{
    // Enable DSDV logs by default. Comment this if too noisy
    // LogComponentEnable("DsdvRoutingProtocol", LOG_LEVEL_ALL); // Update to DSDV
...
DsdvExample::CreateDevices()
{
    ...
    if (pcap)
    {
        wifiPhy.EnablePcapAll(std::string("dsdv")); // Update to DSDV
    }
}
...
{
    DsdvHelper dsdv; // Update to DSDV
    // you can configure DSDV attributes here using dsdv.Set(name, value) // Update to DSDV
    InternetStackHelper stack;
    stack.SetRoutingHelper(dsdv); // has effect on the next Install () // Update to DSDV
    stack.Install(nodes);
    ...
    if (printRoutes)
    {
        Ptr<OutputStreamWrapper> routingStream =
            Create<OutputStreamWrapper>("dsdv.routes", std::ios::out); // Update to DSDV
